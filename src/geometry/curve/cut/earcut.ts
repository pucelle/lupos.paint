...
interface Node {
	i: number // vertex index in coordinates array
	x: number
	y: number // vertex coordinates
...
export function earcut(data: number[], holeIndices: number[], dim: number = 2): number[] {
...
function filterPoints(start: Node, end: Node | null = null) {
...
function earcutLinked(ear: Node | null, triangles: number[], dim: number, minX: number, minY: number, invSize: number, pass: number) {
    if (!ear) return;
    ...
    while (ear!.prev !== ear!.next) {
        let prev: Node = ear!.prev;
        let next: Node = ear!.next;
...
function findHoleBridge(hole: Node, outerNode: Node) {
    ...
    do {
        if (hx >= p.x && p.x >= mx && hx !== p.x &&
            ...
            let tan = Math.abs(hy - p.y) / (hx - p.x); // tangential
...
function sortLinked(list: Node): Node | null {
    let numMerges;
    let inSize = 1;
	let output: Node | null = list

    do {
        let p: Node | null = output;
        let e;
        output = null;
        let tail = null;
        numMerges = 0;
